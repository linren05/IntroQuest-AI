# introquest_app.py
# Ultra-premium UI with minimal sidebar navigation (Option C)
# Run with: streamlit run introquest_app.py
...
# Configuration
# ---------------------------
DEFAULT_RUBRIC_PATH = "/mnt/data/Case study for interns.xlsx"  # local fallback file you uploaded
...
# ---------------------------
# Re-use your robust rubric parser (smart)
# Accepts uploaded file-like object or local path
# ---------------------------
def load_rubric_from_excel_like(obj):
    ...
    df_try = df_raw.copy()
    header_row = None
    for i in range(min(10, df_try.shape[0])):
        row = df_try.iloc[i].astype(str).str.lower().tolist()
    ...
    if header_row is not None:
        ...
        try:
            maybe = pd.read_excel(obj, header=0) if not hasattr(obj, "read") else pd.read_excel(BytesIO(obj.read()), header=0)
        ...
        for _, row in df.iterrows():
            ...
            if crit and crit.lower() not in ["nan", "none"]:
                ...
                if pd.isna(kwcell) or str(kwcell).strip() == "":
                    ...
                    keywords = [k.strip() for k in re.split(r"[;,]", str(kwcell)) if k.strip()]
                try:
...
# ---------------------------
# Scoring building blocks (unchanged)
# ---------------------------
...
def compute_nlp_similarity(transcript, description):
    if not transcript or not description:
        return 0.0
    emb = model.encode([transcript, description], convert_to_tensor=True)
...
def radar_chart(categories, values):
    ...
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 5])), margin=dict(l=10, r=10, t=20, b=10))
    return fig
...
if page == "About":
    ...
    st.markdown("<p>Ultra-premium evaluator combining rule-based checks and NLP semantic similarity. Paste a transcript, upload a rubric Excel (optional), and press Score.</p>", unsafe_allow_html=True)
    st.markdown("<p>Fallback rubric path (local): <code>/mnt/data/Case study for interns.xlsx</code></p>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
else:
